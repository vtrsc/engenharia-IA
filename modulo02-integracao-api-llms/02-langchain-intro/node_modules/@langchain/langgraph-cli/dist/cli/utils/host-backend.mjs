...
export class HostBackendClient {
        ...
        for (let attempt = 0; attempt <= TRANSPORT_RETRIES; attempt += 1) {
            ...
            try {
                ...
                if (attempt < TRANSPORT_RETRIES)
                    continue;
                throw new HostBackendError(error instanceof Error ? error.message : String(error));
            }
            finally {
            ...
            if (!response.ok) {
                ...
                throw new HostBackendError(`${method} ${path} failed with status ${response.status}: ${detail}`, response.status);
            }
            ...
            try {
                ...
                throw new HostBackendError(`Failed to decode response from ${path}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        throw new HostBackendError(lastError instanceof Error ? lastError.message : String(lastError));
    }
    /**
    ...
    createDeployment(args) {
        ...
        if (args.source === "internal_source" && args.configPath) {
            payload.source_revision_config["langgraph_config_path"] = args.configPath;
        }
     ...
     */
    updateDeployment(deploymentId, imageUri, args = {}) {
        const payload = {
     ...
     * @returns The updated deployment object.
     */
    updateDeploymentInternalSource(deploymentId, args) {
        const payload = {
     ...
     */
    listRevisions(deploymentId, limit = 1) {
     ...
     */
    getRevision(deploymentId, revisionId) {
        return this.request("GET", `/v2/deployments/${deploymentId}/revisions/${revisionId}`);
    }
    /**
     ...
     */
    getBuildLogs(projectId, revisionId, payload) {
        return this.request("POST", `/v1/projects/${projectId}/revisions/${revisionId}/build_logs`, { payload });
    }
    /**
     ...
     */
    getDeployLogs(projectId, payload, revisionId) {
