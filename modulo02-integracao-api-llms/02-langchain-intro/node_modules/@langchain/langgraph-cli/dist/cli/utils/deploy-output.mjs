...
export class Emitter {
     ...
     */
    statusChange(status, elapsedSeconds) {
     ...
     * @param elapsedSeconds - Seconds elapsed since polling began.
     */
    heartbeat(status, elapsedSeconds) {
        if (!this.json)
            return;
        this.write({
            event: "heartbeat",
    ...
    uploadProgress(sizeMb, pct) {
        if (this.json) {
            ...
            process.stdout.write(`\r   Uploading (${sizeMb.toFixed(1)} MB)... ${pct}%`);
        }
    }
    /**
    ...
    result(status, args) {
        if (this.json) {
            ...
            if (args.statusUrl)
                payload.status_url = args.statusUrl;
            this.write(payload);
            return;
        }
        if (status === "succeeded") {
            ...
            if (args.url)
                this.echo(colorize(`   URL: ${args.url}`, "green"));
            if (args.statusUrl) {
