    ...
    async function walk(dir, relPrefix) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (ALWAYS_EXCLUDE_NAMES.has(entry.name))
                    continue;
                const dirRel = `${rel}/`;
                ...
                if (spec.ignores(dirRel) && !spec.requiresDirWalk(rel))
                    continue;
                await walk(abs, rel);
                continue;
            }
 ...
 */
export async function createArchive(configPath, options = {}) {
    ...
    try {
        ...
        if (!files.includes(configRel)) {
            throw new Error(`Archive validation failed: ${configRel} not found in archive`);
        }
        await tarCreate({
            gzip: true,
            file: archivePath,
        ...
        if (fileSize > MAX_SIZE) {
            throw new Error(`Source archive is ${(fileSize / BYTES_PER_MIB).toFixed(1)} MB, which ` +
        ...
        if (fileSize > WARN_SIZE && options.onWarn) {
            options.onWarn(`Warning: source archive is ${(fileSize / BYTES_PER_MIB).toFixed(1)} MB. ` +
