 ...
 */
export function hasDisallowedBuildCommandContent(command) {
 ...
 */
export function normalizeImageTag(value) {
    const tag = value || "latest";
    if (!/^[A-Za-z0-9_.-]+$/.test(tag)) {
        throw new Error("Image tag may only contain characters A-Z, a-z, 0-9, '_', '-', '.'");
    }
    return tag;
}
/**
 ...
 */
export function secretsFromEnv(envVars, onSkip) {
    const secrets = [];
 ...
 */
export function envWithoutDeploymentName(envVars) {
...
function extractDeploymentUrl(deployment) {
    ...
    if (sourceConfig && typeof sourceConfig === "object") {
        const customUrl = sourceConfig.custom_url;
 ...
 */
export function formatDeploymentsTable(deployments) {
    const rows = deployments.map((deployment) => [
        String(deployment.id ?? "-") || "-",
        String(deployment.name ?? "-") || "-",
        extractDeploymentUrl(deployment),
    ]);
    return buildTable(["Deployment ID", "Deployment Name", "Deployment URL"], rows);
}
/**
 ...
 * @returns The formatted table string.
 */
export function formatRevisionsTable(revisions) {
 ...
 */
export function smithDashboardBaseUrl(hostUrl) {
    if (!hostUrl)
 ...
 */
export function getDeploymentStatusUrl(updated, deploymentId, hostUrl) {
    const tenantId = updated && typeof updated === "object"
        ? updated.tenant_id
        : undefined;
    if (!tenantId)
        return null;
 ...
 */
export function findDeploymentIdByName(response, name) {
    if (!name)
        return null;
