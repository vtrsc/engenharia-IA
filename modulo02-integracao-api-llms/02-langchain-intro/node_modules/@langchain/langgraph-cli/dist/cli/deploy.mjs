...
import { HostBackendClient, HostBackendError, } from "./utils/host-backend.mjs";
import { Emitter, Spinner } from "./utils/deploy-output.mjs";
import { createArchive, BYTES_PER_MIB } from "./utils/archive.mjs";
import { API_KEY_ENV_NAMES, DEFAULT_HOST_URL, DEPLOYMENT_NAME_ENV, TERMINAL_STATUSES, envWithoutDeploymentName, findDeploymentIdByName, formatDeploymentsTable, formatLogEntry, formatRevisionsTable, getDeploymentStatusUrl, hasDisallowedBuildCommandContent, isPathWithin, levelColor, normalizeImageTag, normalizeName, secretsFromEnv, smithDashboardBaseUrl, } from "./utils/deploy-helpers.mjs";
// ---------------------------------------------------------------------------
...
const COLORS = { cyan: "36", green: "32", yellow: "33", red: "31" };
...
// Config / env / secrets
// ---------------------------------------------------------------------------
function validateDeployCommands(installCommand, buildCommand) {
...
function warnNonWolfiDistro(rawConfig) {
    const distro = rawConfig.image_distro ?? "debian";
    if (distro === "wolfi")
        return;
    emitter.note("⚠️  Security Recommendation: Consider switching to Wolfi Linux for enhanced security.");
    emitter.note("   Wolfi is a security-oriented, minimal Linux distribution designed for containers.");
    emitter.note('   To switch, add \'"image_distro": "wolfi"\' to your langgraph.json config file.');
}
async function resolveEnvPath(rawConfig, configPath) {
    ...
    if (typeof envField === "string") {
        ...
        if (!isPathWithin(projectRoot, envPath)) {
            emitter.note(`Ignoring env file '${envField}' specified in langgraph.json: ` +
                "the path escapes the project directory.");
            return null;
        }
        if (!(await exists(envPath))) {
            emitter.note(`Warning: env file '${envField}' specified in langgraph.json not found.`);
            return null;
        }
        return envPath;
    }
...
async function parseEnvFromConfig(rawConfig, configPath) {
    const envField = rawConfig.env;
    if (isPlainObject(envField) && Object.keys(envField).length) {
        const out = {};
    ...
    if (envPath === null || !(await exists(envPath)))
        return {};
...
async function setEnvKey(envPath, key, value) {
    let content = "";
    try {
...
// ---------------------------------------------------------------------------
async function createHostBackendClient(hostUrl, apiKey, envVars) {
    let resolvedEnv = envVars;
    if (resolvedEnv === undefined) {
        resolvedEnv = await parseEnvFromConfig({}, path.join(process.cwd(), "langgraph.json"));
    }
    ...
    if (!resolvedApiKey) {
        if (noInput) {
            throw new Error("No LangSmith API key found. Set LANGSMITH_API_KEY in the environment or .env file.");
        }
        process.stdout.write(paint("No LangSmith API key found. Create one at Settings > API Keys in LangSmith.\n", "yellow"));
        resolvedApiKey = await promptHidden("Enter LangSmith API key");
    }
    const tenantId = resolvedEnv.LANGSMITH_TENANT_ID || process.env.LANGSMITH_TENANT_ID;
    return new HostBackendClient(hostUrl, resolvedApiKey, tenantId);
}
async function callWithOptionalTenant(client, operation) {
    ...
    for (;;) {
        try {
            ...
            if (!(error instanceof HostBackendError))
                throw error;
            if (!promptedForTenant &&
                error.statusCode === 403 &&
                error.message.includes("requires workspace specification")) {
                if (noInput) {
                    throw new Error("API key is org-scoped and requires a workspace ID. Set LANGSMITH_TENANT_ID " +
                        "in your .env file or use a workspace-scoped API key.");
                }
                process.stdout.write(paint("Your API key is org-scoped and requires a workspace ID.\n", "yellow"));
                process.stdout.write(paint("Find your workspace ID in LangSmith under Settings > Workspaces.\n", "yellow"));
            ...
            if (error.statusCode === 403 &&
                ...
                throw new HostBackendError("LangSmith Deployment is not enabled for this organization. " +
                    `Enable it at ${smithBase}/host/deployments (ensure this matches the ` +
                    "organization for your API key).", 403);
            }
            throw error;
        }
    }
...
// ---------------------------------------------------------------------------
const DOCKER_NOT_INSTALLED = "Docker is required but not installed.\n" +
    "Install Docker Desktop: https://docs.docker.com/get-docker/";
const DOCKER_NOT_RUNNING = "Docker is installed but not running.\nStart Docker and try again.";
async function canBuildLocally() {
    let opts;
    try {
...
async function resolveBuildMode(remoteBuildFlag) {
    const [supported, localBuildError] = await canBuildLocally();
    if (remoteBuildFlag === true)
        return [true, localBuildError];
    if (remoteBuildFlag === false) {
        if (!supported) {
            throw new Error(`${localBuildError || "Unable to build locally."}\n\n` +
...
async function resolveDeployment(client, step, deploymentId, name, notFoundMessage) {
    if (deploymentId) {
    ...
    const response = await callWithOptionalTenant(client, (c) => c.listDeployments(name));
...
async function createDeployment(client, step, args) {
    ...
    if (!id) {
        throw new HostBackendError("POST /v2/deployments succeeded but response missing a valid 'id'");
    }
...
async function pollRevisionStatus(client, deploymentId, options) {
    ...
    if (options.progressMessage)
        spinner.set(options.progressMessage);
    try {
        while (Date.now() < deadline) {
            ...
            const status = typeof rec(rev).status === "string"
                ? rec(rev).status
                : "UNKNOWN";
            ...
            if (options.onPoll) {
                await options.onPoll(status, revisionId, (message) => spinner.set(message));
            }
...
async function printDeploymentResult(client, deploymentId, lastStatus, options) {
    const depInfo = await client.getDeployment(deploymentId);
    const sourceConfig = rec(depInfo).source_config;
    const customUrl = isPlainObject(sourceConfig) && typeof sourceConfig.custom_url === "string"
        ? sourceConfig.custom_url
        : null;
...
// ---------------------------------------------------------------------------
async function dockerConfigForToken(registryHost, token) {
...
async function resolvePushedImageDigest(opts, remoteImage) {
    // rsplit on ":" preserves any ":port" in the registry host.
    const repoNoTag = remoteImage.slice(0, remoteImage.lastIndexOf(":"));
    let digests = [];
    try {
        const { stdout } = await $(opts) `docker image inspect --format ${"{{json .RepoDigests}}"} ${remoteImage}`;
    ...
    emitter.warn(`Could not resolve image digest for ${remoteImage}; falling back to the ` +
...
async function uploadToGcs(signedUrl, filePath, fileSize) {
    ...
    const body = new ReadableStream({
        async pull(controller) {
            ...
            const { bytesRead } = await handle.read(buffer, 0, UPLOAD_CHUNK_SIZE, uploaded);
            if (bytesRead === 0) {
                await closeHandle();
                controller.close();
                return;
    ...
    if (lastPct !== 100)
        emitter.uploadProgress(sizeMb, 100);
    if (!emitter.jsonMode)
        process.stdout.write("\n");
}
async function runLocalBuild(args) {
    ...
    const baseImageRef = args.baseImage ?? getBaseImage(args.config, args.apiVersion);
    const generatedDockerfile = await configToDocker(args.configPath, args.config, localDeps, {
        watch: false,
        dockerCommand: "build",
    ...
    const dockerfile = generatedDockerfile.replace(/^FROM .*$/m, `FROM ${baseImageRef}`);
    ...
    try {
        ...
        if (error instanceof HostBackendError &&
            ...
            error.message.includes("only available for 'internal_docker' source deployments")) {
            throw new Error(`Deployment '${args.deploymentId}' was not created by 'langgraph deploy' ` +
    ...
    const deploymentToken = typeof pushData.token === "string" ? pushData.token : undefined;
    const registryUrl = typeof pushData.registry_url === "string"
    ...
    if (normalizedRegistry.includes("://")) {
        normalizedRegistry = normalizedRegistry.slice(normalizedRegistry.indexOf("//") + 2);
    }
    ...
    const { dir: cfgDir, cleanup } = await dockerConfigForToken(registryHost, deploymentToken);
    try {
        // -- Login --
        ...
        for (let attempt = 0; attempt < maxPushRetries; attempt += 1) {
            ...
            try {
                ...
                if (attempt < maxPushRetries - 1) {
                    emitter.warn(`Push failed, retrying (attempt ${attempt + 2} of ${maxPushRetries})...`);
                }
                else {
                    throw error;
                }
    ...
    const updated = await args.client.updateDeployment(args.deploymentId, resolvedImage, { secrets: args.secrets });
    return {
        updated: rec(updated),
...
async function runRemoteBuild(args) {
    let step = args.step;
    ...
    let configRel;
    try {
        emitter.info(`Archive created (${(archive.fileSize / BYTES_PER_MIB).toFixed(1)} MB)`);
        ...
        const signedUrl = typeof uploadData.upload_url === "string"
            ? uploadData.upload_url
            : undefined;
        const objectPath = typeof uploadData.object_path === "string"
        ...
        const updated = await args.client.updateDeploymentInternalSource(args.deploymentId, {
        ...
        const onPoll = async (status, revisionId, setProgress) => {
            if (!(args.verbose &&
                (status === "AWAITING_BUILD" || status === "BUILDING"))) {
                return;
            }
            try {
                const payload = logOffset
                    ? { order: "asc", limit: 50, offset: logOffset }
                    : { order: "asc", limit: 50 };
                const logsResp = await args.client.getBuildLogs(args.deploymentId, revisionId, payload);
                ...
                for (const entry of entries) {
                    if (entry.message)
                        emitter.log(String(entry.message));
                }
...
async function runDeploy(dockerBuildArgs, opts, remoteBuildFlag) {
    emitter = new Emitter(opts.json);
    noInput = !opts.input;
    emitter.note("Note: 'langgraphjs deploy' is in beta. Expect frequent updates and improvements.");
    if (!opts.json)
        process.stdout.write("\n");
    if (!["dev", "prod"].includes(opts.deploymentType)) {
        throw new Error(`Invalid --deployment-type '${opts.deploymentType}' (expected 'dev' or 'prod').`);
    }
    // -- 1. Preflight --
    ...
    const secrets = secretsFromEnv(envWithoutDeploymentName(envVars), (skipped) => emitter.note(`Skipping reserved env var: ${skipped}`));
    const [useRemoteBuild, localBuildError] = await resolveBuildMode(remoteBuildFlag);
    ...
    let step = 1;
    const resolved = await resolveDeployment(client, step, deploymentId, name, useRemoteBuild
    ...
    const depStatusUrl = emitDeploymentStatusUrl(buildResult.updated, deploymentId, opts.hostUrl);
    if (!opts.wait) {
        emitter.info(buildResult.noResultMessage);
        return;
    }
    const [lastStatus, revisionId] = await pollRevisionStatus(client, deploymentId, {
    ...
    if (buildResult.showBuildLogsOnFailure &&
...
async function runList(opts) {
    ...
    const response = await callWithOptionalTenant(client, (c) => c.listDeployments(opts.nameContains));
    const resources = Array.isArray(rec(response).resources)
        ? rec(response).resources
        : [];
    const deployments = resources.filter(isPlainObject);
...
async function runRevisionsList(deploymentId, opts) {
    emitter = new Emitter(false);
    noInput = false;
    const client = await createHostBackendClient(opts.hostUrl, opts.apiKey);
    const limit = Number.parseInt(opts.limit, 10) || 10;
    const response = await callWithOptionalTenant(client, (c) => c.listRevisions(deploymentId, limit));
    const resources = Array.isArray(rec(response).resources)
        ? rec(response).resources
        : [];
    const revisions = resources.filter(isPlainObject);
    if (!revisions.length) {
...
async function runDelete(deploymentId, opts) {
    emitter = new Emitter(false);
    noInput = false;
    if (!opts.force) {
        const confirmed = await confirm(paint(`Are you sure you want to delete deployment ID ${deploymentId}? (Y/n)`, "yellow"));
        if (!confirmed) {
...
async function runLogs(opts) {
    emitter = new Emitter(false);
    noInput = false;
    const envVars = await parseEnvFromConfig({}, path.join(process.cwd(), "langgraph.json"));
    const client = await createHostBackendClient(opts.hostUrl, opts.apiKey, envVars);
    let name = opts.name;
    ...
    if (opts.type === "build" && !revisionId) {
        ...
        const resources = Array.isArray(rec(revisionsResp).resources)
            ? rec(revisionsResp).resources
            : [];
        if (!resources.length) {
            throw new Error("No revisions found for this deployment. Cannot fetch build logs.");
        }
    ...
    const payload = { limit, order: "desc" };
    if (opts.level)
        payload.level = opts.level.toUpperCase();
    if (opts.query)
    ...
    const fetchLogs = async (requestPayload) => {
        const resp = opts.type === "build"
            ? await client.getBuildLogs(depId, revisionId, requestPayload)
            : await client.getDeployLogs(depId, requestPayload, revisionId);
        const logs = rec(resp).logs;
        return Array.isArray(logs) ? logs : [];
    };
    const printEntries = (entries, reverse = false) => {
    ...
    if (initial.length)
        updateStartTime(initial[0].timestamp);
    let stopped = false;
    process.on("SIGINT", () => {
        stopped = true;
        process.stdout.write("\nStopped.\n");
    ...
    while (!stopped) {
        ...
        const fresh = entries.filter((entry) => !seenIds.has(String(entry.id ?? "")));
        if (fresh.length) {
            printEntries(fresh);
...
const deploy = builder
    .command("deploy")
    .description("[Beta] Build and deploy a LangGraph image to LangSmith Deployment.")
    .option("-c, --config <path>", "Path to configuration file", process.cwd())
    .option("--api-key <key>", "API key (or LANGGRAPH_HOST_API_KEY / LANGSMITH_API_KEY / LANGCHAIN_API_KEY).")
    .option("--name <name>", "Deployment name (or LANGSMITH_DEPLOYMENT_NAME). Defaults to the current directory name.")
    .option("--deployment-id <id>", "ID of an existing deployment to update.")
    .option("--deployment-type <type>", "Deployment type when creating: 'dev' or 'prod'.", "dev")
    ...
    .option("--no-pull", "Do not pull the latest base image before building locally.")
    ...
    .option("--json", "Emit structured JSON-lines to stdout instead of human-readable text.", false)
    .option("--no-input", "Never prompt for input; fail with an error if a required value is missing.")
...
deploy
    ...
    .option("--host-url <url>", "Host backend URL.", DEFAULT_HOST_URL)
    .option("--name-contains <value>", "Only show deployments whose names contain this value.", "")
    .exitOverride((error) => gracefulExit(error.exitCode))
...
revisions
    ...
    .exitOverride((error) => gracefulExit(error.exitCode))
    .action(action(async (deploymentId, opts) => {
...
deploy
    ...
    .option("--type <type>", "Log stream: 'deploy' (runtime) or 'build' (remote build logs).", "deploy")
