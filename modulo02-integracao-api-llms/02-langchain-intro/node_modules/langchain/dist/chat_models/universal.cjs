...
function getLangSmithChatModelParams({ apiKey, configuration = {} }) {
	const gatewayConfig = (0, _langchain_core_utils_gateway.resolveLangSmithGatewayConfig)({
		baseURL: configuration.baseURL,
		providerPath: "v1"
	});
	return {
		apiKey: apiKey ?? configuration.apiKey ?? gatewayConfig.apiKey ?? ((0, _langchain_core_utils_env.getEnvironmentVariable)("LANGSMITH_GATEWAY_API_KEY") || (0, _langchain_core_utils_env.getEnvironmentVariable)("LANGSMITH_API_KEY")),
		configuration: {
			...configuration,
			baseURL: gatewayConfig.baseURL ?? `${_langchain_core_utils_gateway.DEFAULT_LANGSMITH_GATEWAY}/v1`
		},
...
async function _initChatModelHelper(model, modelProvider, params = {}) {
	...
	return new (await (getChatModelByClassName(config.className, modelProviderCopy)))({
		model,
		...passedParams,
		...modelProviderCopy === "langsmith" ? getLangSmithChatModelParams(passedParams) : {}
	});
}
/**
