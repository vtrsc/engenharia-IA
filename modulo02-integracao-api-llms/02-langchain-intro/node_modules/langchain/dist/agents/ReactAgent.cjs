...
const require_tool_call = require("./transformers/tool-call.cjs");
const require_subagent = require("./transformers/subagent.cjs");
	...
	constructor(options, defaultConfig) {
		...
		*/
		...
		for (let i = 0; i < beforeAgentNodes.length; i++) {
			...
			if (node.allowed && node.allowed.length > 0) {
				...
				const destinations = Array.from(/* @__PURE__ */ new Set([nextDefault, ...allowedMapped.map((dest) => dest === _langchain_langgraph.END ? exitNode : dest)]));
		...
		if (afterAgentNodes.length > 0) {
			...
			if (firstAfterAgent.allowed && firstAfterAgent.allowed.length > 0) {
				...
				*/
				const destinations = Array.from(/* @__PURE__ */ new Set([_langchain_langgraph.END, ...allowedMapped]));
		...
		*/
		...
		const compileTransformers = [
			require_tool_call.createToolCallTransformer([]),
			require_subagent.createSubagentTransformer([]),
