	...
	return require_middleware.createMiddleware({
		...
		wrapModelCall: (request, handler) => {
			...
			if (deferNames.size > 0) {
				const available = tools.filter(_langchain_core_utils_function_calling.isLangChainTool).map((t) => t.name);
...
function isDeferred(tool, deferNames) {
	return (0, _langchain_core_utils_function_calling.isLangChainTool)(tool) && (tool.extras?.defer_loading === true || deferNames.has(tool.name));
}
...
//#endregion
exports.providerToolSearchMiddleware = providerToolSearchMiddleware;

//# sourceMappingURL=providerToolSearch.cjs.map