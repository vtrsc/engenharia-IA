	...
	*/
	getFullPath(key) {
		try {
			...
			const rootPath = node_path.resolve(this.rootPath);
			const fullPath = node_path.resolve(rootPath, keyAsTxtFile);
			const relativePath = node_path.relative(rootPath, fullPath);
			if (relativePath === ".." || relativePath.startsWith(`..${node_path.sep}`) || node_path.isAbsolute(relativePath)) throw new Error(`Invalid key: ${key}. Key should be relative to the root path. Root path: ${this.rootPath}, Full path: ${fullPath}`);
			return fullPath;
		} catch (e) {
