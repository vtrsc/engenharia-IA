...
exports.loadTracesForExperiment = loadTracesForExperiment;
...
const v2_migration_js_1 = require("../utils/v2_migration.cjs");
...
async function loadTracesForExperiment(client, project, options) {
    ...
    const runs = (await client._supportsSDBQuery())
        ? await (0, v2_migration_js_1.loadTracesV2)(client, project, { isRoot })
...
async function evaluateComparative(experiments, options) {
    ...
    const experimentRuns = await Promise.all(projects.map((project) => loadTracesForExperiment(client, project, {
