...
class SandboxClient {
     ...
     * @returns Snapshot in "ready" status.
     */
    async createSnapshotFromDockerfile(name, dockerfile, fsCapacityBytesOrOptions, options = {}) {
        const fsCapacityBytes = typeof fsCapacityBytesOrOptions === "number"
            ? fsCapacityBytesOrOptions
            : undefined;
        const resolvedOptions = typeof fsCapacityBytesOrOptions === "number"
            ? options
            : (fsCapacityBytesOrOptions ?? options);
        const { context = ".", buildArgs, target, onBuildLog, vCpus, memBytes, timeout = 60, } = resolvedOptions;
